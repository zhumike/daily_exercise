...
from star_common.gateway.euler_base_rpc import challenge_client
...
def online_process():
    last_id = 0
    while True:
        campaigns = StarCampaignDAL.query(equal_filters={'id__gt': last_id, 'status': 1, 'type': 1}, limit=10,
                                          order_by="id", column=['id', ])
        if len(campaigns) == 0:
            break
        last_id = max([_.id for _ in campaigns])
        for campaign in campaigns:
            CampaignAutoProcessHandler(campaign.id).process()
    return
...
if __name__ == "__main__":
    ...
    #
    # python / opt / tiger / toutiao / app / ad / star_orders / unit_test / campaign / test_auto_create_challenge.py
    # 6986101401369706540
    # 1